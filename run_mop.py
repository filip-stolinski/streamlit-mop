...
uploaded_file = st.sidebar.file_uploader('Upload file', type=["xlsx",'csv'], key="uploaded_file",label_visibility='hidden')
...
if 'load_button' in locals() and (load_button or st.session_state['load_button_clicked']):
    if uploaded_file is not None:
        if uploaded_file.name.split('.')[-1] == 'xlsx':
            ...
            uploaded_file.seek(0) # read_csv in streamlit buffer problem - this is a solution
        ...
        if st.button("Run Conversion"):
            ...
            if output_files:
                ...
                st.markdown(get_binary_file_downloader_html(zip_filename, 'Download ZIP file'), unsafe_allow_html=True)
